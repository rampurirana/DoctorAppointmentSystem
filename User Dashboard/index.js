...
async function loadWelcomeName() {
    ...
    if (userId !== "mock_user") {
        try {
            const response = await fetch(`${window.APP_BACKEND_URL}/api/user/${userId}`);
            if (response.ok) {
...
async function updateNotificationBadge() {
    ...
    try {
        const response = await fetch(`${window.APP_BACKEND_URL}/api/user/${userId}/notifications/unread-count`);
